...
export async function POST(req: Request) {
  try {
    ...
    const blockedEmails = db.collection("blocked_emails");

    // Check if email is blocked
    const blocked = await blockedEmails.findOne({ email });
    if (blocked) {
      return NextResponse.json(
        {
          error:
            "This email is not allowed to register. Please contact support.",
        },
        { status: 403 },
      );
    }
    ...
    response.headers.append(
      "Set-Cookie",
      `has_subscription=false; Path=/; Max-Age=${maxAge}; SameSite=Lax${secure}`,
    );
